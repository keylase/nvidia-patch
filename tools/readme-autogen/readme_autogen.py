...
def linux_readme(data):
    ...
    example_driver = find_driver(
        drivers, linux_driver_key(data["example"]), linux_driver_key
    )
...
def windows_driver_rows(drivers, repo_base):
    ...
    def row_gen():
        for d in drivers:
            ...
            patch64_link = (
                markdown_link_tmpl.substitute(text="x64 library patch", url=patch64_url)
                if patch64_url
                else ""
            )
            patch32_link = (
                markdown_link_tmpl.substitute(text="x86 library patch", url=patch32_url)
                if patch32_url
                else ""
            )
            driver_link = (
                markdown_link_tmpl.substitute(text="Driver link", url=driver_url)
                if driver_url
                else ""
            )
            date_patch_available = d.get("date_patch_available", "N/A")
            date_driver_release = d.get("date_driver_release", "N/A")
            yield driver_row_tmpl.substitute(
                product=product,
                version_variant=version_variant,
                patch64_link=patch64_link,
                patch32_link=patch32_link,
                driver_link=driver_link,
                date_driver_release=date_driver_release,
                date_patch_available=date_patch_available,
            )
...
def windows_driver_table(drivers, repo_base):
    ...
    def section_gen():
        for k, g in groupby(drivers, lambda d: WinSeries[d["os"]]):
            ...
            yield os_section_tmpl.substitute(os=os, product_sections=product_sections)
...
def windows_readme(data, repo_base):
    ...
    geforce_drivers = filter(
        lambda d: Product[d["product"]] is Product.GeForce, drivers
    )
    quadro_drivers = filter(lambda d: Product[d["product"]] is Product.Quadro, drivers)
    latest_geforce_version = max(
        geforce_drivers, default="xxx.xx", key=lambda d: version_key_fun(d["version"])
    )["version"]
    latest_quadro_version = max(
        quadro_drivers, default="xxx.xx", key=lambda d: version_key_fun(d["version"])
    )["version"]
    return master_tmpl.substitute(
...
def parse_args():
    ...
    parser.add_argument(
        "-R", "--repo-root", help="repository web root URL", default=REPO_BASE
    )
